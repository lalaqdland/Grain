...
**状态**：待开发（Ready for Local Development）
...
- **算法策略**：
  ...
  - **英文**：**混合架构**。优先使用**MarianMT（本地模型）**进行`En -> De -> En`回译以引入自然噪音；辅以DeepSeek进行Human-like润色。
...
| 后端 | FastAPI（Python） | **本地Uvicorn** | 负责业务逻辑、Word解析、DeepSeek API调用、MarianMT运行，`uvicorn main:app --reload`启动 |
